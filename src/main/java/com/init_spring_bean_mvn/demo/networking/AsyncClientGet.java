    ...
    public static void main(String[] args) {
        try {
            ...
            //while(!responseFuture.isDone()) {
                while((response = responseFuture.getNow(null)) == null) { // HttpResponse type of Stream of Strings
                    // parm value is value of response future not being done yet - getNow has to be sAME type that gets returned - HttpResponse<Stream<String>>
                    // get method with timeout
                    // while(true) {
                        // try {
                   //  response = responseFuture.get(1, TimeUnit.SECONDS); THROWS EXCEPTION IF RESPONSE NOT RETURNED BEFORE TIMEOUT, EXECUTIONEXCEPTION AND TIMEOUTEXCEPTION ARE THROWN WITH TIEOUTEXCPETION print . in while loop and try again
                    //  more fine tuned control of what happened - keeps thread available for other work while CompleteableFutre is not yet done.
                    //NEXT use methods on CompleteableFuture that take callbacks
                    System.out.print(". ");
                TimeUnit.SECONDS.sleep(1); // won't get here until response returns from GET method - Solution: asyncSend instead of Threading
