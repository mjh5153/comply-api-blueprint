...
public class JavaNetworkingMultiThreaded {
    public static void main(String[] args) {
        ...
        try(ServerSocket sst = new ServerSocket(5000)) {
            ...
            while(true) {
                    Socket socket = sst.accept(); // only first connection will be accepted, one thread per client connection
                // Less scalable for handling large numbers, TCP

                    // 386: refactored for multiple network
                ...
                // NIO type prefferent with Channels instead of sockets - superior scalability and efficiency, trade off - complexity

                // ServerSocketChannel - Non-blocking I?O, one thread can handle multiple client connections efficiently, stream based or datagram based,
                // potentially faster due to non-blocking and efficient resource utilization
                // Application suitability - perfect for high performance needs
                //  Complexity - Demonstrated - buffers, channels and selectors
                    System.out.println("Server taking requests ");

                    socket.setSoTimeout(900_000); // drop client after 15 minutes// closes client connection and type of io connection exceptiong ets thrown
                    executorService.submit(() -> handleClientRerquest(socket));
                    // server will hold dropped connection
...
//                        String echoString = input.readLine(); // blocking code
//
...
//                    }
            ...
            System.out.println("Server try resources multi solution excetpion " + e.getMessage());

        }

    }

    // fix readLine timeout issue
    private static void handleClientRerquest(Socket socket) {
        try (socket;
              // block here unless terminate application
             //
              BufferedReader input = new BufferedReader(new InputStreamReader(socket.getInputStream()))) {


            PrintWriter output = new PrintWriter(socket.getOutputStream(), true); //io streams will get closed by outer try catch) { // cose socket and resources cleanly

            System.out.println("Handle client request: ");
            // closing server socket just won't accept new connections
            while (true) { // waiting for client to connect on port 5000
                String echoString = input.readLine(); // blocking code
                System.out.println("Echo Input from server: " + echoString); // block here unless terminate application
