...
public class HttpExamplePost {
    public static void main(String[] args) {
        try {
            ...
            urlConnection.setDoOutput(true); // if comment out see cannot set UrlConnection if setDoOutput false
            urlConnection.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
            String parameters ="first=Joe&last=Smith";
            int length = parameters.getBytes().length;
            urlConnection.setRequestProperty("Content-Length",String.valueOf(length)); // comment out to test if also required - Will be successful but want
            // to inform the browser to ensure clarity and consitiency if clarity isn't obvious
            ...
            if(responseCode != HTTP_OK) {
                ...
                System.out.printf("Error: %s%n", urlConnection.getResponseMessage());
                // Error
                //Error: Not Found
                //text/html
