    ...
    public static void main(String[] args) {
        try {
            ...
            //while(!responseFuture.isDone()) {
                 // won't get here until response returns from GET method - Solution: asyncSend instead of Threading
                // CompletableFuture<HttpResponse<String>> responseFuture = client.sendAsync(request.HttpResponse.BodyHandler.ofString());
                // Synchronous - HttpResponse<T> returned - determined by parameter BodyHandler<T>
                // CompletableFuture is container for type - rFuture represents result of asynch operation
                // extends Futures to support asynchronous processing - explicitly completed by setting value and status - supporting dependent functions and actions that trigger upon its completion
                // Can use methods on class that support callbacks - pass function as arg to other function - in future, i.e lambda expressions don'tn get executed until method is invoked
                // other defer of stored functions -
                // Java doesn't use term callback in own docs
            responseFuture.thenApply(AsyncHandlerClient::filterResponse) // not doing anything with stream of string yet - need to call last method in thenApply
                    .thenApply(AsyncHandlerClient::transformResponse)
                    .thenAccept(AsyncHandlerClient::printResponse) // Need thenAccept because of return type being void of printResponsecan't chain because take arguments
            .thenRun(() -> {
                for (int i =0; i< 10; i++ )
                    System.out.println(i);
            })
                    .thenRun(System.out::println);
            // after response is processed run final proccss

            System.out.println(" 10 jobs to do besides handling response.");
            // Method reference can be used
            // depends on requirements - writing to file or building large data set so multitasking
            // main thread didn't have to handle the job- once pass callback to thenAccept - ONce main thread finishes callback may not complete
            // or get executed - depends on requirements if main thread nees to have knowledge of callback - difference subtle

            // Without callback - response would never get process - callback processes completion apart for main running code - no requirement for main thread to wait
            // process to handle response occurred while the below while

            // Application needs to remain running long enough for event to occur and callback get executed
            // Can still do work whiele waiting and can define what to do while waiting
            // ** See java api  - CompletableFuture. thenAccept - determined by jvm
            // callback or threadpool - callback lightweight - can stick to non async
            // Can pass completion stage to some methods - implementation of completable stage - can pass completable stages of both jointly
            // Function Interfaces - takes functional lambda as parameter and runnable - overloaded

            // Next - Multiple concurrent completable futures - Next video
            int jobs = 0; // Expected output - Job 1 Job 2 Job 3 Job 4 Job 5 Job 6 Job 7 Job 8 Job 9 Job 10
            while(jobs++ < 10) {
...
//            }
            ...
            // NEXT - blocks until thread returns
            // response = responseFuture.join();
            // NEXT - use getNow - can use in while loop - can combine operations
...
//alt-svc=[h3=":443"; ma=86400]
    ...
    private static Stream<String> filterResponse(HttpResponse<Stream<String>> response) {
        System.out.println("Filtering response " + response.uri());
        if (response.statusCode() == HTTP_OK) {
