...
public class PostConcurrentRequeststoServer {

    private static final Path orderTracking = Path.of("orderTracking.json");
    // write file lock - any thread writing to file, will first require lock
    ...
    public static void main(String[] args) {
        ...
        // sendGets(client, sites); // will be processed before posts
        //sendPosts(client, urlBase, urlParams, orderMap);
        sendPostsSafeFileWrite(client, urlBase, urlParams, orderMap);

        // handle with custom body method - i.e thread safe
        // In video - Create ThreadSafeFileHandler implements HttpResponse.BodyHandler<Path> {
    ...
    // Asynchronous Thread SAFE write to file
    ...
    private static void sendPosts(HttpClient client, String baseUri, String parameters, Map<String,Integer> orders) {

        // User post builder method,
        // send each post request asynch - list of Com;eteableFutures
        // use allOf - create aggregated CF - use join method

        // Goal - send 5 async post requests in body
        var futures = orders.entrySet().stream()
                .map(e -> parameters.formatted(e.getKey(), e.getValue()))// now have stream of strings - next need uris
                .map(s -> HttpRequest.newBuilder(URI.create(baseUri))
                        .POST(HttpRequest.BodyPublishers.ofString(s)))// Sends 5 posts with body in post request
    ...
    private static void sendPostsSafeFileWrite(HttpClient client, String baseUri, String parameters, Map<String,Integer> orders) {
        ...
        var futures = orders.entrySet().stream()
                ...
                .map(request -> client.sendAsync(
                        request, HttpResponse.BodyHandlers.ofString()
                ).thenAcceptAsync(r -> writeToFile(r.body())) // don't use join , called after response complete, response passed as parameter
                ).toList(); // collect completable futures into list

        // callback leveraged for addtional responses - make acsync as well
        var allFutureRequests = CompletableFuture.allOf(
                futures.toArray(new CompletableFuture<?>[0])
        );

        // Processing large number requests - Async Callback has Performance improvements!!!!
...
//        }
    ...
    private static void sendGets(HttpClient client, List<URI> uris) {
        var futures = uris.stream()
                ...
                .map(HttpRequest.Builder::build)
                .map(request -> client.sendAsync(
                        request, HttpResponse.BodyHandlers.ofString() // NEXT - 406: do thread safe write to File using BodyHandlers.ofString() to avoid Dead Lock
                )).toList(); // collect completable futures into list
        var allFutureRequests = CompletableFuture.allOf(
                futures.toArray(new CompletableFuture<?>[0])
        );
